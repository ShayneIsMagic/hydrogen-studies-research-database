/**
 * Enhanced Data Cleanup Processor for Hydrogen Studies
...
class EnhancedDataCleanupProcessor {
    constructor() {
    ...
    separateContentFromStudies(rawText) {
        ...
        if (parts.length >= 2) {
            ...
            studies = this.extractStudiesFromHTML(studiesText);
        } else {
    ...
    /**
     * Extract studies from HTML text
     */
    extractStudiesFromHTML(text) {
        if (!text) return [];
        
        const studies = [];
        
        // Look for study links with more comprehensive patterns
        const linkPatterns = [
            // Standard study links
            /<a[^>]*href="([^"]*)"[^>]*>([^<]+)<\/a>/gi,
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (linkText && 
                    (href.includes('study') || 
                     ...
                     href.includes('/201') ||
                     href.includes('/200') ||
                     href.includes('hydrogenstudies.com'))) {
     ...
     */
    process() {
        this.log('Starting enhanced data cleanup process...');
        ...
        const outputFilename = `enhanced_cleaned_topics_content_${timestamp}.csv`;
        this.saveCSVToFile(csvContent, outputFilename);
        
        this.log(`Enhanced cleanup complete! Processed ${rawData.length} topics with ${totalStudies} total studies`);
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = EnhancedDataCleanupProcessor;
}

// Run if called directly
if (require.main === module) {
    const processor = new EnhancedDataCleanupProcessor();
    
    const result = processor.process();
    
    if (result && result.success) {
        console.log('\n✅ Enhanced data cleanup completed successfully!');
        ...
        console.log('\n📋 The enhanced cleaned CSV now contains:');
